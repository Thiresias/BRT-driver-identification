    ...
    def forward(self, wa, alpha, feats):
        ...
        if alpha is not None:
            ...
            if len(alpha) > 1:
                ...
                # print(f" >target: {directions_target.shape}  |  source: {directions_source.shape}")
                # directions_start = self.direction(alpha[2])  # start
                # latent = wa + (directions_target - directions_start) + directions_source
                # directions_target = directions_target - directions_source

            else:
                directions_target = self.direction(alpha[0])
